...
失败 episode 处理原则、采集规模和评估协议已经完成第一轮规划。当前下一步不直接写完整采集程序，而是先用自然语言描述：

```text
一次 episode 从场景重置、逐步采集到结束保存，依次发生哪些事情
```

暂时不考虑函数名、类和目录拆分。先把流程说清楚，再在单个 `collect_episode.py` 中做最小实现。
...
确认需要跨脚本复用时再拆到 src
参数多到影响阅读时再建立配置文件
真正产生数据、模型和日志时再启用对应目录
```

因此，现有目录是项目未来可能生长成的工程骨架，不代表当前必须一次理解、实现或填满所有目录。
...
在固定机位、固定背景和固定光照下，
完成红蓝方块分类放置的真实机械臂闭环，
并通过固定测试协议和数据量对比实验量化系统表现。
```

完成项目必须同时满足：

```text
1. demonstration 能够通过遥操作采集并按 episode 保存
2. Dataset Loader 能正确对齐 image / robot_state / action
3. BC baseline 能训练、保存和加载
4. policy 能在真实机械臂上闭环执行
5. 红蓝任务各完成 10 轮正式测试
6. 记录分类、抓取、放置和端到端成功率
7. 完成 20 条训练数据与 60 条训练数据的对比
8. 记录失败类型、平均完成时间和平均步数
9. 完成 README、实验表、失败案例和 Demo 视频
```

ACT 属于时间允许时的增强项，Diffusion Policy 不属于 2027 年 3 月前第一版的必做内容。

## 21. Schedule To March 2027

考研前具身智能每周固定投入 3 小时，采用周内 3 次 30 分钟加周日 90 分钟。考研仍是主线，考前重点消除软件结构和数据流程的不确定性，考后集中处理真机调试。

```text
2026 年 9 月：恢复主线，完成 episode 自然语言流程
2026 年 10 月：完成单文件模拟 collect_episode
2026 年 11 月：完成模拟 Dataset Loader 和 BC 闭环
2026 年 12 月至初试：收束文档、硬件与官方接口方案
初试后第 1-2 周：跑通机械臂、相机和遥操作
初试后第 3-4 周：采集首批 40 条成功 episode，完成 BC 真机闭环
2027 年 2 月上半月：扩充至 60 条，完成 20 条与 60 条数据对比
2027 年 2 月下半月：完成 20 轮评估、失败分析、README 和 Demo
```
...
周内短时学习负责理解、接口和小函数
周日负责运行、验证和记录
未完成的必做闭环优先于 ACT / Diffusion Policy
每次结束记录完成项、阻塞项和下一次第一步
```
